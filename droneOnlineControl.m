...
if use_GP == 1
    %% Load hyper parameter file from data/hyper
    loadError = 0;
    while loadError == 0
        hypFile = input('Load hyper parameter file: ', 's');
        try
            load( strcat( dataPath, hyperAdd, hypFile) );
            loadError = 1;
        catch
            loadError = 0;
            disp('File not found. Try another hyper parameter file name.');
        end
    end
    fprintf('-------------------------------------------------------------\n');
    
    %% Load alpha matrix file from data/alpha
    alphaFiles = dir( strcat(dataPath, alphaAdd, 'alpha*.mat') );
    for i = 1:length(alphaFiles)
        fprintf('%40s   [%2d]\n', alphaFiles(i).name, i)
    end
    
    loadError = 0;
    while loadError == 0
        alphaFileIdx = input('Select Alpha File: ');
        try
            alphaFileName = alphaFiles(alphaFileIdx).name ;
            load( strcat( dataPath, alphaAdd, alphaFileName ) );
            fprintf('Alpha Matrix Size:   %dx%d \n', size(alpha,1), size(alpha,2) );
            loadError = 1;
        catch
            loadError = 0;
            disp('Index does not match. Select one of [index] for alpha names shown above.');
        end
    end
    fprintf('-------------------------------------------------------------\n');
    
    %% Load training data from data/trainSampled folder
    trainSampledName = strcat( 'trainSampled', alphaFileName(6:end) );
    fprintf('Loading trained data: %50s\n', trainSampledName);
    load( strcat( dataPath, sampledAdd, trainSampledName) ) ;
    
    %% Determining number of states for training data
    stateFlag = 0;
    while stateFlag ~= 4 && stateFlag ~= 6 && stateFlag ~= 9 && stateFlag ~= 12
        stateFlag   = input('# of states for training [4, 6, 9, 12] : ');
    end
    if stateFlag == 4
        states  = [1:3,9];  % x, y, z, psi
    elseif stateFlag == 6
        states  = 1:6;      % phi, theta, psi, vx, vy, vz
    elseif stateFlag == 9
    ...
    %% Determining training data for online control | xp = trainData
...
for k = 1:1
    tic
    fprintf('Set up reference for trajectory:       %2d     |   ', k);
    %     clf;
    ...
    for i=1:length(x_ref)
        %     i
        tStart = tic;
        %% Get xq based on states [4, 6, 9, or 12]
        if use_GP == 1
            xq = droneState(states);
            for queryIdx = 1:numTrainSamples
                kernel(queryIdx) = calcKernel(hyp, trainData(queryIdx,:), xq);
            end
            kAlpha = kernel*alpha;
        end
        ...
        %% If QP is applied only for Tau-x and Tau-y, DO NOT USE THIS
        if use_QP_u1 == 1 || use_QP_txy == 0
            % u1 = m*sqrt(xdd_cmd^2 + ydd_cmd^2 + (zdd_cmd-g)^2);
            if use_GP == 1
                xdd_pred = xdd_cmd - kAlpha(1);
                ydd_pred = ydd_cmd - kAlpha(2);
                zdd_pred = zdd_cmd - kAlpha(3);
            else
                xdd_pred = xdd_cmd;
                ydd_pred = ydd_cmd;
                zdd_pred = zdd_cmd;
            end
            u1 = m*sqrt(xdd_pred^2 + ydd_pred^2 + (zdd_pred-g)^2);
        end
...
%             end
            ...
            I = [Ix Iy Iz];
            [w1, w2, w3, w4] = setAngularVelocities(kAlpha, u1, pd_cmd, qd_cmd, rd_cmd, droneState, I, kf, km, l);
            w_total = w1 + w2 + w3 + w4;
    ...
    if (plot_flag == 1)
        ...
        if model_wind == 1
        quiver3(wind_x+1, wind_y-1, wind_z-5, wu/4, wv/4, ww/2, ...
               'Color', [0.25 0.75 0], ...
               'LineWidth'  , 5.0, ...
               'MaxHeadSize', 1.5, ...
               'MarkerSize' , 10) ; %, 'MaxHeadSize', 10.5);
        end
